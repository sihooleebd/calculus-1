...
#canvas.cartesian-canvas(
  ...
  graph.graph(x => 2 * (x - 1) + 1, domain: (0, 2), style: (stroke: red)),
  // Point P(1, 1)
  graph.point(1, 1, label: $(1, 1)$),
  //Tangent line at point P(2, 4)
  graph.graph(x => 4 * (x - 2) + 4, domain: (1, 3), style: (stroke: green)),
  // Point P(2, 4)
  graph.point(2, 4, label: $(2, 4)$),
  //Tangent line at point P(3, 9)
  graph.graph(x => 6 * (x - 3) + 9, domain: (2, 4), style: (stroke: blue)),
  // Point P(3, 9)
  graph.point(3, 9, label: $(3, 9)$),
)
...
#canvas.cartesian-canvas(
  ...
  show-grid: true,
  // Graph of f(x) = |x|
  graph.graph(x => calc.abs(x), domain: (-2, 2)),
  // Point P(0, 0)
...
#definition("Non-Differentiable Point")[
  ...
  #canvas.cartesian-canvas(
    ...
    show-grid: true,
    // Graph of f(x) = x^(1/3)
    graph.graph(x => calc.root(x, 3), domain: (-2, 2)),
    // Point P(0, 0)
    graph.point(0, 0, label: $(0, 0)$),
  )
]
