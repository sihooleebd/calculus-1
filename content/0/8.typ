...
#definition("Continuity at a Point")[
  ...
  1. $f(a)$ is defined.
  2. $limits(lim)_(x -> a) f(x)$ exists.
  3. $limits(lim)_(x -> a) f(x) = f(a)$

  Equivalently, for every number $epsilon > 0 $ there is a number $delta > 0$ #st
  $ abs(x-a)<delta ==> abs(f(x)-f(a)) < epsilon $
]
...
#definition("Right Continuity at a Point")[
  ...
  Right side limits appear frequently in probability theory, while left continuity are less commonly used.
]
...
#definition("Continuity on an Interval")[
  ...
  #note("Remark")[
    If $f$ is defined only on one side of an endpoint of the interval, we understand continuous at the endpoint to mean continuous from the right or left continuity.
  ]
]
...
#theorem()[
  ...
  these are all continuous. trig functions will be revisited.
]
...
#example()[
  Evaluate
  $ limx(pi) (sin x)/(2  + cos x) $
  #solution()[
    $sin x$ and $2 + cos x$ are continuous at $x=pi$ and the denom isn't 0 thus continuous. Plugging in,
    $ therefore 0/3 = 0 $
  ]
]

Are limit and composed function interchangeable?

#example("Counter example")[
  Let $f(x) = H(x)$ (Heaviside), $g(x) = sin x$
  $ f(limx(0) g(x) = f(0) = 1) $
  However,
  $ limx(0^-)f(sin x) = 0, wide limx(0^+)f(sin x) = 1 $
  Duh.
]

#theorem()[
  If $f$ is continuous at $b$ and $limx(a) g(x) = b$ then $limx(a) g(f(x))=f(b)$.

  In other words,
  $ limx(a)f(g(x)) = f(limx(a)g(x)) $

  #proof()[
    Let $epsilon>0$. There exists $delta_1, delta_2>0$ #st
    $ abs(y-b)<delta_1 &==> abs(f(y)-f(b))<epsilon, \ 0<abs(x-a)<delta_2 &==> abs(g(x)-b)<delta_1 $

    Choose $delta = delta_2$.
    Then for all $epsilon > 0$,

    $ 0<abs(x-a)<delta_2 ==> abs(f(g(x))-f(g(a))) $
  ]
]
...
#theorem("Intermediate Value Theorem")[
  ...
  Equivalently, \
  Suppose that $f$ is continuous on the colosed interval $[a,b]$ and that $f(a) $ and $f(b) $ have opposite signs, i.e., $f(a)f(b) < 0$, then there exists $c in (a,b)$ #st $f(c) = 0$.
  #note("Warning")[
    The Intermediate Value Theorem *does not* tell us how many solutions exists. It guarantees only the existence of the solution.
  ]
  #proof("")[
